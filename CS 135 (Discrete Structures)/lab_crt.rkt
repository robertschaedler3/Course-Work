...
(define (CRT-exists-helper? tuplelist b blist)
  (if (equal? (cdr tuplelist) '())
      #t
      (if (equal? blist '())
          (CRT-exists-helper? (cdr tuplelist) (car (cdr (car (cdr tuplelist)))) (cdr (cdr tuplelist)))
          (if (equal? (EuclidAlgo b (car (cdr (car blist)))) 1)
              (CRT-exists-helper? tuplelist b (cdr blist))
              #f))))

(define (CRT-exists? tuplelist)
  (CRT-exists-helper? tuplelist (car (cdr (car tuplelist))) (cdr tuplelist)))
...
(define (CRT-helper tuplelist m)
  (if (equal? (cdr tuplelist) '())
      (* (car (car tuplelist)) (* (/ m (car (cdr (car tuplelist)))) (mul_inv (/ m (car (cdr (car tuplelist)))) (car (cdr (car tuplelist))))))
...
(define (CRT tuplelist)
 (if (CRT-exists? tuplelist)
      (if (> (CRT-helper tuplelist (M tuplelist)) 0)
          (CRT-helper tuplelist (M tuplelist))
          (+ (CRT-helper tuplelist (M tuplelist)) (CRT-helper2 M tuplelist 0)))
      -1))

(define (CRT-helper2 M tuplelist curr_num)
  (if (< curr_num (* -1 (CRT-helper tuplelist (M tuplelist))))
      (CRT-helper2 M tuplelist (+ curr_num (M tuplelist)))
      curr_num))
