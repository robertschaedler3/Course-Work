...
(define (union listA listB)
  (if (null? listA)
      listB
      (make-set (cons (car listA) (union (cdr listA) listB)))))
...
(define (set-equal? setA setB)
  (if (and (null? setA) (null? setB))
      #t
      (if (and (subset? setA setB) (subset? setB setA))
          #t
          #f)))
...
(define (set-difference listA listB)
  (if (null? listA)
      listA
      (if (not (element? (car listA) listB))
          (make-set (cons (car listA)(set-difference (cdr listA) listB)))
          (make-set (set-difference (cdr listA) listB)))))
...
(define (disjoint? setA setB)
  (if (null? (intersection setA setB))
      #t
      #f))
