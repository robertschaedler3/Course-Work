...
(define (convert10 number base)
  (if (zero? number)
      '()
      (reverse (cons (modulo number base) (reverse (convert10 (quotient number base) base))))))
...
(define (base10 oldbase numlist)
  (if (null? numlist)
      0
      (base10Helper oldbase 1 (reverse numlist) 0)))


(define (base10Helper oldbase multiplier numlist num)
  (if (null? numlist)
      num
      (base10Helper oldbase (* oldbase multiplier) (cdr numlist) (+ num (* multiplier (car numlist))))))
...
(define (geo-prog cur_num ratio n)
  (geoHelper cur_num ratio n (list cur_num)))

(define (geoHelper cur_num ratio n l)
   (if (< n 2)
       l
       (geoHelper (* cur_num ratio) ratio (- n 1) (append l (list (* cur_num ratio))))))
